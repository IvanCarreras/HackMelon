...
def escaneoRed():
    ...
    for iface in interfaces:
        if(iface.is_valid()):
            ...
            print(f"{colors.WHITE}Identificador de Red:{bcolors.ENDC} {iface.network_name}")
            print(f"{colors.WHITE}Descripcion:{bcolors.ENDC} {iface.description}")
...
def escaneoPuertos():
    ...
    for host in scanner.all_hosts():
        ...
        uptime_str = str(timedelta(seconds=uptime_seconds)) if uptime_seconds else "N/A"
        ...
        for proto in scanner[host].all_protocols():
            ...
            for port in sorted(ports):
                ...
                detalles = " ".join(filter(None, [product, version, f"({extrainfo})" if extrainfo else ""])).strip()
...
def fuzzing():
    ...
    diccionario = "./assets/directory-list-2.3-medium.txt"
    ...
    try:
        ...
        for linea in wordlist:
            ...
            if response.status_code == 200: # OK
                tqdm.write(f"/{linea} {bcolors.OKCYAN}(Status: 200){bcolors.ENDC}")

            elif response1.status_code == 200:
                tqdm.write(f"/{linea}.txt {bcolors.OKCYAN}(Status: 200){bcolors.ENDC}")

            elif response2.status_code == 200:
                tqdm.write(f"/{linea}.html {bcolors.OKCYAN}(Status: 200){bcolors.ENDC}")

            elif response.status_code == 301: # URL Moved permanently
                tqdm.write(f"{linea} {bcolors.WARNING}(Status: 301){bcolors.ENDC}")

            elif response1.status_code == 301:
                tqdm.write(f"{linea}.txt {bcolors.WARNING}(Status: 301){bcolors.ENDC}")

            elif response2.status_code == 301:
                tqdm.write(f"{linea}.html {bcolors.WARNING}(Status: 301){bcolors.ENDC}")
...
def CVEWiki():
    ...
    if response.status_code == 200:
        ...
        # Extraer gravedad
        severity_tag = soup.find('a', attrs={"data-testid": "vuln-cvss3-panel-score"})
        severity = severity_tag.text.strip() if severity_tag else "No encontrada"

        # Extraer fecha de descubrimiento
        published_date_tag = soup.find('span', attrs={"data-testid": "vuln-published-on"})
        published_date = published_date_tag.text.strip() if published_date_tag else "No encontrada"
...
# https://xss-game.appspot.com/level1/frame?query=
    url = "http://localhost:8000/Web%20pruebas/index.html?name="
    wordlist = "./assets/xss-payload-list.txt"
    ...
    try:
        barrita = tqdm(total=len(payloads),desc="Progreso", unit="urls", dynamic_ncols=True)
        isVuln = False
